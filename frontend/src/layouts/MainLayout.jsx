...
const MainLayout = () => {
  ...
  // Auto-expand sidebar on hover if collapsed
  ...
  // Cancel auto-expand and collapse on mouse leave
  const handleSidebarMouseLeave = () => {
    ...
    // If sidebar was auto-expanded by hover, collapse it again
    const savedState = localStorage.getItem('sidebarCollapsed');
    if (savedState === 'true') {
  ...
  const navigation = [
    { name: 'Dashboard', href: '/', icon: BarChart, admin: true },
    { name: 'Students', href: '/students', icon: Users, admin: true },
    { name: 'QR Scanner', href: '/scanner', icon: QrCode, public: true },
    { name: 'Attendance', href: '/attendance', icon: Calendar, admin: true },
    { name: 'Reports', href: '/reports', icon: BarChart, admin: true },
    { name: 'WhatsApp', href: '/whatsapp', icon: MessageCircle, admin: true },
    { name: 'Settings', href: '/settings', icon: Settings, admin: true },
    { name: 'Profile', href: '/profile', icon: UserCircle, public: true },
  ];

  // Add navigation guard for admin routes
  useEffect(() => {
    const adminRoutes = ['/whatsapp', '/settings', '/reports'];
    if (isAuthenticated && !isAdmin && adminRoutes.includes(location.pathname)) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-0 ${theme === 'dark' ? 'dark bg-slate-900' : 'bg-gray-50'}`}>
      {/* Mobile sidebar */}
      <AnimatePresence>
        {sidebarOpen && (
          <div className="fixed inset-0 z-40 flex md:hidden">
            ...
            >
              ...
              <div className="flex flex-shrink-0 items-center px-6">
                <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-blue-400 dark:from-blue-400 dark:to-blue-300 logos">Attendance System</h1>
              </div>
                ...
                {isAuthenticated ? (
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate max-w-[120px]">{user?.email}</p>
                    </div>
      ...
      <div 
        ...
        className={`hidden md:fixed md:inset-y-0 md:flex md:flex-col ${sidebarCollapsed ? 'md:w-20' : 'md:w-64'} transition-all duration-300 ease-in-out`}
      >
        <div className="flex min-h-0 flex-1 flex-col border-r border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 transition-colors duration-200 shadow-md dark:shadow-slate-900/50 relative">
          ...
          {/* Logo */}
          <div className={`flex flex-shrink-0 items-center h-16 px-4 ${sidebarCollapsed ? 'justify-center' : 'justify-between'} overflow-hidden border-b border-gray-200 dark:border-slate-700`}>
            {sidebarCollapsed ? (
              <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-blue-400 dark:from-blue-400 dark:to-blue-300">AS</h1>
            ) : (
              <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-blue-400 dark:from-blue-400 dark:to-blue-300 logos">Attendance System</h1>
            )}
          </div>
          ...
          <div className="flex flex-1 flex-col overflow-y-auto pt-5 pb-4">
            <nav className={`mt-2 flex-1 space-y-1 ${sidebarCollapsed ? 'px-2' : 'px-3'}`}>
              {filteredNavigation.map((item, index) => (
                <motion.div
                  key={item.name}
                  custom={index}
                ...
                >
                  <Link
                    to={item.href}
                    className={`group flex items-center ${sidebarCollapsed ? 'justify-center p-3' : 'px-3 py-2.5'} text-sm font-medium rounded-lg ${
                      location.pathname === item.href
                        ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 shadow-sm'
                        : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/50 hover:text-gray-900 dark:hover:text-white'
                    }`}
                  ...
                  >
                    <item.icon
                      className={`${sidebarCollapsed ? 'mr-0' : 'mr-3'} ${sidebarCollapsed ? 'h-6 w-6' : 'h-5 w-5'} flex-shrink-0 ${
                        location.pathname === item.href
                          ? 'text-blue-600 dark:text-blue-400'
                          : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-500 dark:group-hover:text-gray-300'
                      }`}
          ...
          {/* User menu */}
          <div className={`border-t border-gray-200 dark:border-slate-700 ${sidebarCollapsed ? 'p-3' : 'p-4'}`}>
            {isAuthenticated ? (
              <div className="relative">
                <div 
                  className={`w-full flex ${sidebarCollapsed ? 'justify-center' : 'items-center text-left'} cursor-pointer`} 
                ...
                {isAuthenticated && (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.97 }}
                    onClick={handleLogout}
                    className={`mt-3 w-full flex items-center justify-center ${sidebarCollapsed ? 'p-2' : 'py-2 px-4'} bg-gradient-to-r from-red-600 to-red-500 dark:from-red-700 dark:to-red-600 text-white rounded-lg hover:from-red-700 hover:to-red-600 dark:hover:from-red-800 dark:hover:to-red-700 shadow-sm transition-all duration-200 cursor-pointer`}
                    role="button"
                    tabIndex={0}
                    title="Logout"
                    aria-label="Logout"
                ...
                <AnimatePresence>
                  {showUserDropdown && (
                    <motion.div 
                      className={`absolute ${sidebarCollapsed ? 'left-full ml-2' : 'right-0 left-0'} mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-10`}
                      initial="hidden"
                      animate="visible"
                      exit="hidden"
      ...
      {/* Main content - Updated to adjust for sidebar state */}
      <div className={`flex flex-1 flex-col ${sidebarCollapsed ? 'md:pl-20' : 'md:pl-64'} transition-all duration-300 ease-in-out`}>
        {/* Top header */}
        <div className="sticky top-0 z-10 flex h-16 flex-shrink-0 bg-white dark:bg-slate-800 shadow dark:shadow-slate-700/20 md:hidden transition-colors duration-200">
          ...
          <div className="flex flex-1 justify-between items-center px-4">
            <h1 className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-blue-400 dark:from-blue-400 dark:to-blue-300">Attendance System</h1>
