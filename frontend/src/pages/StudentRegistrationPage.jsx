...
import { AlertCircle, Info, ArrowLeft, ArrowRight, CheckCircle } from 'lucide-react';
...
const StudentRegistrationPage = () => {
    ...
    switch (name) {
      ...
      case 'indexNumber':
        if (!value.trim()) {
          ...
          newErrors.indexNumber = 'Index number must be less than 20 characters';
        } else {
      ...
      case 'parent_telephone':
        if (!value.trim()) {
          ...
          if (!/^\+?\d{10,15}$/.test(cleanNumber)) {
            newErrors.parent_telephone = 'Phone number must be 10-15 digits with optional + prefix';
        } else {
  ...
  const registerStudent = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to register student. Please try again.';
  ...
  const downloadQRCode = () => {
    try {
      ...
      link.href = image;
      link.download = `qrcode-${registeredStudent?.indexNumber || formData.indexNumber}.png`;
  ...
  // Field components for cleaner JSX
  const renderTextField = (name, label, type = "text", required = false, placeholder = "", autoComplete = "") => (
    <div className="col-span-1">
      ...
      {errors[name] && touched[name] ? (
        ...
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 h-4">&nbsp;</p>
      )}
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center justify-center">
            ...
            <div className={`h-1 w-32 transition-all duration-500 ${step >= 2 ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
            <div className="flex items-center relative">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
          ...
          {step === 1 ? (
            ...
            >
              <form onSubmit={registerStudent} className="space-y-8">
                <div className="border-b border-gray-200 dark:border-gray-700 pb-4 mb-6">
                  <h2 className="text-lg font-medium text-gray-800 dark:text-gray-100">Personal Information</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Enter the student's basic information</p>
                </div>

                <div className="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2">
                  {renderTextField("name", "Full Name", "text", true, "John Doe")}
                  {renderTextField("indexNumber", "Index Number", "text", true, "S12345")}
                  {renderTextField("student_email", "Student Email", "email", true, "student@example.com", "email")}
                  {renderTextField("address", "Address", "text", false, "123 Street, City", "street-address")}
                </div>

                <div className="border-b border-gray-200 dark:border-gray-700 pb-4 mb-6">
                  <h2 className="text-lg font-medium text-gray-800 dark:text-gray-100">Parent/Guardian Information</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Enter parent or guardian contact details</p>
                </div>

                <div className="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2">
                  {renderTextField("parent_email", "Parent Email", "email", true, "parent@example.com", "email")}
                  {renderTextField("parent_telephone", "Parent Telephone", "tel", true, "+94XXXXXXXXXX", "tel")}
            ...
            >
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-semibold text-gray-800 dark:text-white mb-2">Registration Successful!</h2>
                <p className="text-gray-600 dark:text-gray-300 mb-6">
                  Student has been registered successfully. You can now download their QR code.
                </p>
              </div>
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-5 border border-gray-100 dark:border-gray-600 shadow-sm">
                    <dl className="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900 dark:text-white font-medium">{registeredStudent?.name || formData.name}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Index Number</dt>
                        <dd className="mt-1 text-sm text-gray-900 dark:text-white font-medium">{registeredStudent?.indexNumber || formData.indexNumber}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Email</dt>
                        <dd className="mt-1 text-sm text-gray-900 dark:text-white font-medium">{registeredStudent?.student_email || formData.student_email}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Address</dt>
                        <dd className="mt-1 text-sm text-gray-900 dark:text-white font-medium">{registeredStudent?.address || formData.address}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Age</dt>
                        <dd className="mt-1 text-sm text-gray-900 dark:text-white font-medium">{registeredStudent?.age || formData.age}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Registration Date</dt>
                        <dd className="mt-1 text-sm text-gray-900 dark:text-white font-medium">{new Date().toLocaleDateString()}</dd>
                      </div>
                    </dl>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center mb-5">
                    This QR code contains the student's information and can be used for attendance tracking.
                  </p>
                  <motion.button
              ...
              >
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  You can share this QR code with the student for attendance marking.
                </p>
                <div className="flex justify-end space-x-3">
                  <motion.button
                    whileHover={{ scale: 1.02 }}
