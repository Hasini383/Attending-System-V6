...
import { Users, CheckCircle, ArrowRightCircle, XCircle, RefreshCw, AlertCircle, Calendar } from 'lucide-react';
...
const StatsCard = ({ title, count, total, icon }) => (
  <div className="bg-gradient-to-br from-white to-blue-50 dark:from-gray-800 dark:to-gray-700 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-blue-100 dark:border-gray-600 transform hover:-translate-y-1">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-sm font-medium text-blue-600 dark:text-blue-300">{title}</p>
        <p className="text-3xl font-bold text-blue-900 dark:text-white mt-2">
          ...
          {total !== undefined && <span className="text-blue-400 dark:text-blue-400 text-lg ml-1">/ {total}</span>}
        </p>
      </div>
      ...
      <table className="min-w-full divide-y divide-blue-100 dark:divide-gray-700">
        <thead>
          <tr className="bg-gradient-to-r from-blue-600 to-blue-400 dark:from-blue-800 dark:to-blue-600">
            ...
            <th className="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Index Number</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Email</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Status</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Entry Time</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Leave Time</th>
          </tr>
        </thead>
        <tbody className="bg-white dark:bg-gray-800 divide-y divide-blue-50 dark:divide-gray-700">
          {students.map((student, index) => (
            <tr 
              ...
              className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-blue-50 dark:bg-gray-700'} hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors duration-150`}
            >
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center">
                  <div className="flex-shrink-0 h-10 w-10 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center">
                    <span className="text-blue-600 dark:text-blue-300 font-semibold">{student.name?.charAt(0)?.toUpperCase() || '?'}</span>
                  </div>
                  <div className="ml-4">
                    <div className="text-sm font-medium text-blue-900 dark:text-blue-100">{student.name || 'N/A'}</div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-700 dark:text-blue-300 font-mono">{student.indexNumber || 'N/A'}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-500 dark:text-blue-400">{student.student_email || student.email || 'N/A'}</td>
              <td className="px-6 py-4 whitespace-nowrap">
...
const DatePicker = ({ selectedDate, setSelectedDate, isValidDate }) => {
  ...
  const formattedSelectedDate = DateTime.fromJSDate(selectedDate).toFormat('yyyy-MM-dd');
    ...
    <label htmlFor="date" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
      ...
      <div className="relative">
        ...
        <input
          ...
          max={today}
          className={`w-full pl-10 px-3 py-2 border ${isValidDate ? 'border-gray-300 dark:border-gray-600' : 'border-red-300 dark:border-red-700'} rounded-lg focus:ring-2 ${isValidDate ? 'focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400' : 'focus:ring-red-500 focus:border-red-500 dark:focus:ring-red-400 dark:focus:border-red-400'} dark:bg-gray-700 dark:text-white`}
        />
        {!isValidDate && (
      ...
      {!isValidDate && (
        <p className="mt-1 text-sm text-red-600 dark:text-red-400">Cannot select a future date</p>
      )}
    </div>
  );
};
...
const AttendanceByDatePage = () => {
  ...
  const fetchAttendanceByDate = useCallback(async (date = selectedDate) => {
    if (!isValidDate) {
      setAttendanceData([]);
      setStats({
        totalStudents: 0,
    ...
    try {
      ...
      // This is the key change that will give us the leave time data
      ...
      // Create a lookup map from the full data response
      ...
      const processedData = students.map(student => {
        ...
        // Try to get the complete student data including leave time
        const fullStudent = studentId && fullStudentMap[studentId] ? fullStudentMap[studentId] : student;
        ...
        // Or use the attendance record from the full data if available
        const fullAttendanceRecord = !attendanceRecord && fullStudent.attendanceHistory?.find(record => {
          if (!record.date) return false;
        ...
        if (record?.entryTime) {
          try {
            ...
            console.warn(`Invalid entry time format for student ${student.indexNumber}:`, record.entryTime);
          }
        }
        ...
        if (record?.leaveTime) {
          try {
            ...
            console.warn(`Invalid leave time format for student ${student.indexNumber}:`, record.leaveTime);
          }
        }
        
        // Use the latest status from the record or student
        ...
        switch (normalizedStatus) {
          case 'entered':
            ...
            normalizedStatus = 'present'; // Match Dashboard behavior
            break;
          case 'left':
            displayStatus = 'Left';
            break;
          ...
          case 'absent':
            ...
            break;
          default:
            displayStatus = latestStatus?.charAt(0).toUpperCase() + latestStatus?.slice(1) || 'Unknown';
        ...
        const processedStudent = {
          ...
          indexNumber: student.indexNumber?.toUpperCase() || fullStudent.indexNumber?.toUpperCase() || 'N/A',
          student_email: student.email || student.student_email || fullStudent.email || fullStudent.student_email || 'N/A',
          status: normalizedStatus,
          displayStatus: displayStatus,
          entryTime: entryTimeObj,
          leaveTime: leaveTimeObj,
          timestamp: student.timestamp || student.updatedAt || fullStudent.timestamp || fullStudent.updatedAt || new Date().toISOString()
        };
      ...
      processedData.forEach(student => {
        ...
        // If this student isn't in our map yet, add them
        if (!studentMap[key]) {
          studentMap[key] = {
            ...student,
        ...
        // If the student is already in our map, update their record
        const existingStudent = studentMap[key];
        
        // Keep the most recent entry time - ensure proper comparison of Date objects
        if (student.entryTime && (!existingStudent.entryTime || 
            (student.entryTime instanceof Date && existingStudent.entryTime instanceof Date && 
        ...
        // Keep the most recent leave time - ensure proper comparison of Date objects
        if (student.leaveTime && (!existingStudent.leaveTime || 
            (student.leaveTime instanceof Date && existingStudent.leaveTime instanceof Date && 
        ...
        // Update status based on most recent scan
        if (student.timestamp && (!existingStudent.timestamp || 
      ...
      const sortedData = mergedData.sort((a, b) => {
        ...
        const statusPriority = { entered: 1, present: 1, left: 2, late: 3, absent: 4 };
      ...
      // Calculate stats
      const presentStudents = mergedData.filter(s => s.status === 'entered' || s.status === 'present').length;
      const leftStudents = mergedData.filter(s => s.status === 'left').length;
      const absentStudents = mergedData.filter(s => s.status === 'absent').length;
      const lateStudents = mergedData.filter(s => s.status === 'late').length;
      ...
      const errorType = err.message === 'Network Error' || !err.response ? 'network' : 'server';
      ...
      // Only show toast if we haven't shown one for this error type
      if (errorToastRef.current !== errorType) {
        ...
        toast.error(
          ...
          {
            onClose: () => {
              // Clear the ref when the toast is closed
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 w-[200px] md:w-auto">
            <DatePicker selectedDate={selectedDate} setSelectedDate={setSelectedDate} isValidDate={isValidDate} />
            <button
