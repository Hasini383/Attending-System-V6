...
import { Pencil, Trash, Plus, QrCode, Users, X, Clock } from 'lucide-react';
import { studentService } from '../services/api';
import { toast } from 'react-toastify';
import { motion, AnimatePresence } from 'framer-motion';
import { useTheme } from '../context/ThemeContext';
import StylishQRCode from '../components/scanner/StylishQRCode';
import QRCodeWithLoading from '../components/scanner/QRCodeWithLoading';
...
const StudentsPage = () => {
  ...
  return (
    ...
    >
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <div className="flex justify-end space-x-2">
                              <motion.button
                                ...
                                whileTap={{ scale: 0.9 }}
                                onClick={() => openQRCodeModal(student)}
                                className="text-purple-600 dark:text-purple-400 hover:text-purple-900 dark:hover:text-purple-300"
                                title="View QR Code"
                              >
                                <QrCode className="h-5 w-5" aria-hidden="true" />
                                <span className="sr-only">QR Code for {student.name}</span>
                              </motion.button>
                              <motion.button
                                whileHover={{ scale: 1.1 }}
                                whileTap={{ scale: 0.9 }}
      ...
      {/* Edit Student Modal */}
      {showEditModal && (
        <div className="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="edit-student-modal" role="dialog" aria-modal="true">
              ...
              <div className="bg-white dark:bg-slate-800 px-6 pt-6 pb-6 sm:p-8 relative">
                <div className="text-center sm:text-left">
                  <div className="flex items-center mb-4">
                    ...
                    >
                      <Pencil className="h-6 w-6 text-blue-600 dark:text-blue-400" aria-hidden="true" />
                    </div>
                    <h3 className="text-xl leading-6 font-bold text-gray-900 dark:text-white">Edit Student</h3>
                  </div>
                  
                  <div className="mt-4">
                    <form onSubmit={updateStudent} className="space-y-4">
                      <div>
                        <label htmlFor="edit-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">
                          Name
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label htmlFor="edit-indexNumber" className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">
                          Index Number
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label htmlFor="edit-address" className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">
                          Address
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label htmlFor="edit-student_email" className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">
                          Student Email
                        </label>
                        <input
                          type="email"
                      ...
                      <div>
                        <label htmlFor="edit-parent_email" className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">
                          Parent Email
                        </label>
                        <input
                          type="email"
                      ...
                      <div>
                        <label htmlFor="edit-parent_telephone" className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">
                          Parent Phone
                        </label>
                        <input
                          type="text"
      ...
      {/* QR Code Modal */}
      {showQRCode && selectedStudentForQR && (
        <div className="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="qr-code-modal" role="dialog" aria-modal="true">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div 
              className="fixed inset-0 bg-black bg-opacity-60 dark:bg-opacity-80 transition-opacity" 
              aria-hidden="true" 
            ...
            <div 
              className="inline-block overflow-hidden shadow-2xl transform sm:my-8 sm:align-middle sm:max-w-md sm:w-full relative z-50 rounded-xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="bg-white dark:bg-slate-800 px-6 pt-6 pb-6 sm:p-8 relative">
                ...
                <div className="text-center pb-2">
                  <div
                    className="inline-flex items-center mb-3 bg-purple-100 dark:bg-purple-900/30 p-2 rounded-full"
                  >
                    <QrCode className="h-6 w-6 text-purple-600 dark:text-purple-400" />
                  </div>
                  
                  <h3 
                    className="text-xl leading-6 font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent dark:from-blue-400 dark:to-indigo-400 mb-1"
                  >
                  ...
                  >
                    Student Index: {selectedStudentForQR.indexNumber}
                  </p>
                  
                  <div 
                    className="flex justify-center"
                  >
                    <div className="p-1 bg-white rounded-xl shadow-lg border-2 border-gray-100 dark:border-slate-700">
                      <QRCodeWithLoading studentId={selectedStudentForQR._id} studentName={selectedStudentForQR.name} size={220} />
                    </div>
                  </div>
                  
                  <div 
                    className="mt-5 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-800"
                  >
                    <p className="text-sm text-blue-600 dark:text-blue-300">
                      Scan this QR code using the attendance system to mark attendance for this student.
                    </p>
                  </div>
                </div>
              </div>
              
              <div className="bg-gray-50 dark:bg-slate-700/50 px-6 py-4 sm:px-8">
                <button
                  type="button"
                  className="w-full inline-flex justify-center items-center rounded-xl border border-transparent shadow-sm px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-base font-medium text-white hover:from-blue-600 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-slate-800 sm:text-sm transition-all duration-200"
                  onClick={closeQRCodeModal}
                >
                  Done
                </button>
      ...
      {showDeleteModal && studentToDelete && (
        <div className="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="delete-student-modal" role="dialog" aria-modal="true">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div 
              className="fixed inset-0 bg-black bg-opacity-60 dark:bg-opacity-80 transition-opacity" 
              aria-hidden="true" 
              ...
              <div className="bg-white dark:bg-slate-800 p-8 relative">
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex-shrink-0 flex items-center justify-center h-16 w-16 rounded-full bg-red-100 dark:bg-red-900/30 sm:mx-0 sm:h-14 w-14 border-2 border-red-200 dark:border-red-800 shadow-inner">
                    <Trash className="h-8 w-8 text-red-600 dark:text-red-400" aria-hidden="true" />
                  </div>
                ...
                <div className="mt-5 bg-gray-50 dark:bg-slate-700/30 p-6 rounded-xl border border-gray-200 dark:border-slate-700">
                  <div className="flex items-center mb-4">
                    ...
                    <span className="text-base font-medium text-gray-900 dark:text-white">Student Information</span>
                  </div>
                  <div className="ml-7 space-y-2">
                    <div>
                      <span className="font-semibold text-gray-700 dark:text-gray-300">Name:</span>
                      <span className="ml-2 text-gray-900 dark:text-white">{studentToDelete.name}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-gray-700 dark:text-gray-300">Index:</span>
                      <span className="ml-2 text-gray-900 dark:text-white">{studentToDelete.indexNumber}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-gray-700 dark:text-gray-300">Email:</span>
                      <span className="ml-2 text-gray-900 dark:text-white">{studentToDelete.student_email}</span>
                    </div>
                  </div>
                </div>
                
                <div 
                  className="mt-5 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800"
                >
                  <div className="flex items-start">
                    <svg className="h-5 w-5 text-red-600 dark:text-red-400 mt-0.5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                    </svg>
                    <p className="text-sm text-red-600 dark:text-red-300">
                      <span className="font-semibold">Warning:</span> This will delete all data associated with this student, including attendance records, reports, and personal information. This action cannot be undone.
                    </p>
                  </div>
                </div>
